...
```{r linear models}
mm_resp_null <- lmer(ordernorm_resp_rate ~ 1 + (1|vial_ID), data= resp_all.data)
summary(mm_resp_null)
#that null looks good

mm_resp_1 <- lmer(ordernorm_resp_rate ~ N_species + N_dose + temp + food + sym_state + e_coli_plate + (1|vial_ID), data= resp_all.data)
summary(mm_resp_1)
# the factors that are not significant and I will drop: Nspecies, food

mm_resp_2 <- lmer(ordernorm_resp_rate ~ N_dose + temp + sym_state + e_coli_plate + (1|vial_ID), data= resp_all.data)
summary(mm_resp_2)
emm_resp_2=emmeans(mm_resp_2, specs=pairwise ~ sym_state)
summary(emm_resp_2)
...
mm_resp_3 <- lmer(ordernorm_resp_rate ~ N_dose  + temp + sym_state + e_coli_plate + N_dose*temp + N_dose*sym_state + sym_state*temp + (1|vial_ID), data= resp_all.data)
summary(mm_resp_3)

mm_resp_4 <- lmer(ordernorm_resp_rate ~ N_dose  + temp + sym_state + e_coli_plate +  N_dose*sym_state + sym_state*temp + (1|vial_ID), data= resp_all.data)
summary(mm_resp_4)

mm_resp_5 <- lmer(ordernorm_resp_rate ~ N_dose  + temp + sym_state + e_coli_plate + sym_state*temp + (1|vial_ID), data= resp_all.data)
summary(mm_resp_5)

mm_resp_6 <- lmer(ordernorm_resp_rate ~ N_dose  + temp + sym_state + e_coli_plate +  N_dose*sym_state + (1|vial_ID), data= resp_all.data)
summary(mm_resp_6)
